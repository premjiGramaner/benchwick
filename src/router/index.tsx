...
const RouterComponent: React.FC<IRouterProps> = (props) => {
            ...
            <AuthWrapper {...props}>
                <Routes>
                    {UnAuthMenuList.map((route) => {
                        return (<Route path={route.path} key={getKey()} element={<route.component {...props} routeInfo={route} />} />)
                    })}
                </Routes>
            </AuthWrapper>);
    }
    ...
    return (
        <React.Fragment>
            <Suspense fallback={<Fragment><p>Loading...</p></Fragment>}>{authRoutes}</Suspense>
        </React.Fragment>
    );
};
